...
export default async function handler(req, res) {
  ...
  // GET /api/gas-sync?mobile=01XXXXXXXXX — used by the Profile modal to
  // look up an already-registered user before saving, so re-onboarding on
  // a new device (or re-typing) doesn't create a conflicting duplicate row.
  if (req.method === 'GET') {
    const mobile = req.query && req.query.mobile;
    if (!mobile) return res.status(400).json({ ok: false, error: 'mobile query param required' });
    try {
      const r = await fetch(GAS_URL + '?mobile=' + encodeURIComponent(mobile));
